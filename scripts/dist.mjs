...
for (const key of ['staffTransmog', 'spearTransmog', 'addTalents', 'keeperAttr']) cfgText = cfgText.replace(new RegExp(`^${key} = .*$`, 'm'), `${key} = 0`);
...
// the tool's own keys, visible in the shipped config (F7 cycles saved looks by default)
for (const [key, val, comment] of [
  ['keeperOutfits', '0', 'TransmogKeeper only: saved looks "Name=ids;Name=ids" (see wukong-transmog outfits)'],
  ['keeperOutfitKey', 'F7', 'TransmogKeeper only: key that puts on the next saved look in game, e.g. F7 or Ctrl+F7 (None = off)'],
]) {
  if (new RegExp(`^${key} = `, 'm').test(cfgText)) cfgText = cfgText.replace(new RegExp(`^${key} = .*// Builds the release folder: dist/Transmog & Buff Tool/
//   node/node.exe              official Node.js runtime (downloaded from nodejs.org, checksum verified)
//   src/, node_modules/        the tool and its one dependency
//   game/b1/Binaries/Win64/    the in-game part, taken from a working install (--game <folder>):
//       version.dll, CSharpLoader/ (loader files), CSharpLoader/Mods/TrueWukong/ (patched mod,
